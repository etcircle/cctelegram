...
cctelegram — Telegram bot that bridges Telegram Forum topics to Claude Code sessions via tmux windows. Each topic is bound to one tmux window running one Claude Code instance.
...
./scripts/restart.sh                  # Restart the cctelegram service after code changes
cctelegram hook --install                  # Auto-install Claude Code SessionStart hook
```
...
- Config directory: `~/.cctelegram/` by default, override with `CCTELEGRAM_DIR` env var.
...
Auto-install: `cctelegram hook --install`
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "hooks": [{ "type": "command", "command": "cctelegram hook", "timeout": 5 }]
      }
    ]
  }
}
