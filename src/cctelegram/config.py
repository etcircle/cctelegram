...
.env loading priority: local .env (cwd) > $CCTELEGRAM_DIR/.env (default ~/.cctelegram).
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.tmux_session_name = os.getenv("TMUX_SESSION_NAME", "cctelegram")
        ...
        # Priority: CCTELEGRAM_CLAUDE_PROJECTS_PATH > CLAUDE_CONFIG_DIR/projects > default
        custom_projects_path = os.getenv("CCTELEGRAM_CLAUDE_PROJECTS_PATH")
        ...
        self.show_user_messages = (
            os.getenv("CCTELEGRAM_SHOW_USER_MESSAGES", "true").lower() != "false"
        )
        ...
        self.show_tool_calls = (
            os.getenv("CCTELEGRAM_SHOW_TOOL_CALLS", "true").lower() != "false"
        )

        # Show hidden (dot) directories in directory browser
        self.show_hidden_dirs = (
            os.getenv("CCTELEGRAM_SHOW_HIDDEN_DIRS", "").lower() == "true"
        )
        ...
        try:
            self.tool_summary_max_chars = int(
                os.getenv("CCTELEGRAM_TOOL_SUMMARY_MAX_CHARS", "40")
            )
        except ValueError:
        ...
        # setting CCTELEGRAM_BUSY_INDICATOR_V2=false.
        self.busy_indicator_v2 = (
            os.getenv("CCTELEGRAM_BUSY_INDICATOR_V2", "true").lower() == "true"
        )
        ...
        try:
            self.context_pct_threshold = int(
                os.getenv("CCTELEGRAM_CONTEXT_PCT_THRESHOLD", "80")
            )
        except ValueError:
        ...
        self.context_in_message_footer = (
            os.getenv("CCTELEGRAM_CONTEXT_IN_MESSAGE_FOOTER", "true").lower() == "true"
        )
        ...
        try:
            self.attention_question_preview_chars = int(
                os.getenv("CCTELEGRAM_ATTENTION_QUESTION_PREVIEW_CHARS", "200")
            )
        except ValueError:
        ...
        try:
            self.agent_prompt_preview_chars = int(
                os.getenv("CCTELEGRAM_AGENT_PROMPT_PREVIEW_CHARS", "400")
            )
        except ValueError:
        ...
        self.attention_buttons = (
            os.getenv("CCTELEGRAM_ATTENTION_BUTTONS", "true").lower() != "false"
        )
        ...
        try:
            self.attention_button_ttl_seconds = int(
                os.getenv("CCTELEGRAM_ATTENTION_BUTTON_TTL_SECONDS", "86400")
            )
        except ValueError:
        ...
        self.reply_context_enabled = (
            os.getenv("CCTELEGRAM_REPLY_CONTEXT", "true").lower() != "false"
        )
        ...
        try:
            self.quote_injection_max_chars = int(
                os.getenv("CCTELEGRAM_QUOTE_INJECTION_MAX_CHARS", "1600")
            )
        except ValueError:
        ...
        try:
            self.aggregator_debounce_seconds = float(
                os.getenv("CCTELEGRAM_AGGREGATOR_DEBOUNCE_SECONDS", "1.5")
            )
        except ValueError:
        ...
        raw_max_attachments = os.getenv("CCTELEGRAM_AGGREGATOR_MAX_ATTACHMENTS")
        try:
        ...
        try:
            self.max_attachment_size_bytes: int = int(
                os.getenv("CCTELEGRAM_MAX_ATTACHMENT_SIZE_BYTES", str(20 * 1024 * 1024))
            )
        except ValueError:
            self.max_attachment_size_bytes = 20 * 1024 * 1024

        # §2.5.3 Stage 5.c: Telegram message-refs SQLite. The DB file lives
        # under the config dir by default so it follows the rest of CCTelegram's
        # state files; CCTELEGRAM_MESSAGE_REFS_DB_PATH overrides for tests / split
        # storage volumes.
        message_refs_path = os.getenv("CCTELEGRAM_MESSAGE_REFS_DB_PATH")
        ...
        try:
            self.message_refs_retention_days = int(
                os.getenv("CCTELEGRAM_MESSAGE_REFS_RETENTION_DAYS", "30")
            )
        except ValueError:
        ...
        try:
            self.message_ref_text_max_chars = int(
                os.getenv("CCTELEGRAM_MESSAGE_REF_TEXT_MAX_CHARS", "4000")
            )
        except ValueError:
        ...
        # from. Previously defaulted to Path.cwd(), which surfaced CCTelegram's
        # own repo when restarted from inside the project tree — surprising
        # and OS-coupled. Override with CCTELEGRAM_BROWSE_ROOT to pin a specific
        # workspace directory.
        browse_root_env = os.getenv("CCTELEGRAM_BROWSE_ROOT")
