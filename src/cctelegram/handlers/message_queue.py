...
async def _emergency_dm(
    ...
    prefix = (
        f"⚠️ CCTelegram could not post this {kind} in topic {thread_id} ({display}) "
...
def _context_pct_suffix(pct: int | None) -> str:
    ...
    Below ``CCTELEGRAM_CONTEXT_PCT_THRESHOLD`` (or unknown): empty string.
...
def _render_activity_digest(
    ...
    *,
    ...
    With ``CCTELEGRAM_BUSY_INDICATOR_V2=true`` and a ``route``, the header is
