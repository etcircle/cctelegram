"""Shared utility functions used across CCTelegram modules.

Provides:
  - app_dir(): resolve config directory from CCTELEGRAM_DIR env var.
...
CCTELEGRAM_DIR_ENV = "CCTELEGRAM_DIR"


def app_dir() -> Path:
    """Resolve config directory from CCTELEGRAM_DIR or default ~/.cctelegram."""
    raw = os.environ.get(CCTELEGRAM_DIR_ENV, "")
    return Path(raw).expanduser() if raw else Path.home() / ".cctelegram"
