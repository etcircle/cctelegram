...
Responsibilities:
  - Persist/load state to ~/.cctelegram/state.json.
...
# is still surfaced — that's the whole point of CCTELEGRAM_SHOW_USER_MESSAGES.
...
class SessionManager:
    ...
    async def _cleanup_stale_session_map_entries(self, live_ids: set[str]) -> None:
        ...
        When windows are closed externally (outside CCTelegram), session_map.json
    ...
    async def load_session_map(self) -> None:
        ...
        Keys in session_map are formatted as "tmux_session:window_id" (e.g. "cctelegram:@12").
