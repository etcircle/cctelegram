"""Tests for cctelegram.utils: app_dir, atomic_write_json, read_cwd_from_jsonl."""
...
from cctelegram.utils import atomic_write_json, app_dir, read_cwd_from_jsonl


class TestAppDir:
    def test_returns_env_var_path(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setenv("CCTELEGRAM_DIR", "/custom/config")
        assert app_dir() == Path("/custom/config")

    def test_returns_default_without_env(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.delenv("CCTELEGRAM_DIR", raising=False)
        assert app_dir() == Path.home() / ".cctelegram"
